...
const AppMenu = () => {
    ...
    let columns = [ 
        ...
        //{ title: 'Creation Date', field: 'creationDate', editable: 'never' },
    ]
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        let errorList = []  
        if (newData.appName === "") {
            errorList.push("Try Again, Please enter App Nme")
        }
        if (newData.createdBy === "") {
            errorList.push("Try Again, Please enter Created By")
        } 
        if (newData.activeStatus === "") {
            errorList.push("Try Again, Please Select Active Status")
        } 

        if (errorList.length < 1) {

            axios.put(`http://localhost:4531/appMenu`, newData)
                .then(response => { 
    ...
    const handleRowDelete = (oldData, resolve) => {
        axios.delete(`http://localhost:4531/appMenu/${oldData.appId}`)
            .then(response => {
    ...
    const handleRowAdd = (newData, resolve) => {
        ...
        if (errorList.length < 1) { 
            axios.post(`http://localhost:4531/appMenu`, newData)
                .then(response => { 
            ...
            >
                <Card classes={{ root: classes.cardRoot }}>
                    ...
                    <MaterialTable
                        ...
                        icons={tableIcons}
                        options={{
                            headerStyle: { borderBottomColor: '#f5365c', borderBottomWidth: '3px', },
